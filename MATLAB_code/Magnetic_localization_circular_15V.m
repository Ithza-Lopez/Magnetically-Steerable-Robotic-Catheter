...
x_all = [repelem((dim_repeat),4), repelem((zeros(1,3)),4), repelem((-dim_repeat),4),  repelem((zeros(1,3)),3)]*1e-3; % 6 zeros and alternating x pos and neg
y_all = [repelem(-37.5,3),repelem(-12.5,3),repelem(12.5,3),repelem(37.5,3),repelem(-37.5,3),repelem(-12.5,3),repelem(12.5,3),repelem(37.5,3),repelem(-37.5,3),repelem(-12.5,3),repelem(12.5,3),repelem(37.5,3),repelem(-12.5,3),repelem(12.5,3),repelem(37.5,3)]*1e-3; % decreasing from 126.4 to 51.4 and increaseing to 126.4.
z_all = [repelem((zeros(1,3)),4), repelem((dim_repeat),4),repelem((zeros(1,3)),4), repelem((-dim_repeat),3)]*1e-3; %alternating z pos and neg, and zeros
...
pm_hat_0 = [-0.010;0.080;0.020;theta; phi-deg2rad(tilt_angle);M_exp];   % (x_hat0,y_hat0,z_hat0,theta_hat0,phi_hat0,M0)
...
for i = 1:length(col_labels)
    ...
    for row = num_rows
        ...
        B_meas = B_meas([1:36,40:end]);  % remove sensor 13 measurement
