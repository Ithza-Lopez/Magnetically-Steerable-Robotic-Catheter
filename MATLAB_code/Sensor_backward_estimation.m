function [xyz_s,M] = Sensor_backward_estimation(y_meas,pm_all,xyz_s_0,meas_dir_s,M_0)
...
J = Analytic_Jacobian_Sensor_cir_array(pm_all,xyz_s_0,meas_dir_s,M_0);
...
while ( ~stop && iteration <= MaxIter)
    ...
    if ((max(abs(delta)./(abs([xyz_s;M])+1e-20))) < epsilon_2  &&  iteration > 2 )
    ...
    xyz_s_try = xyz_s + delta(1:3);
    M_try = M + delta(4);
    
    % evaluate value using pm_try
    B_meas_try = Sensor_forward(pm_all,xyz_s_try,meas_dir_s,M_try);  % compute measurement based on given direction
    ...
    if rho > epsilon_4   % if better
        ...
        J = Analytic_Jacobian_Sensor_cir_array(pm_all,xyz_s,meas_dir_s,M);
    else      % if not better
        lambda = lambda*v;
        v = 2*v;
    end
