...
x_all = [repelem((dim_repeat),4), repelem((zeros(1,1)),4), repelem((-dim_repeat),4),  repelem((zeros(1,1)),3)]*1e-3; % 6 zeros and alternating x pos and neg
y_all = [repelem(-37.5,1),repelem(-12.5,1),repelem(12.5,1),repelem(37.5,1),repelem(-37.5,1),repelem(-12.5,1),repelem(12.5,1),repelem(37.5,1),repelem(-37.5,1),repelem(-12.5,1),repelem(12.5,1),repelem(37.5,1),repelem(-12.5,1),repelem(12.5,1),repelem(37.5,1)]*1e-3; % decreasing from 126.4 to 51.4 and increaseing to 126.4.
z_all = [repelem((zeros(1,1)),4), repelem((dim_repeat),4),repelem((zeros(1,1)),3), repelem((-dim_repeat),4)]*1e-3; %alternating z pos and neg, and zeros
...
for i = 1:length(col_labels)
    ...
    for j = num_rows
        ...
        pm_all = [Single_PM_Location(magnet_type, col_letter, j, height_num);theta_pm;phi_pm];
...
for i = 1:length(col_labels)
    ...
    for j = num_rows
        ...
        % collect measurement
        filename = append(exp_date,'-',magnet_type, '-',col_letter,'-',num2str(j),'-', num2str(tilt_angle),'-',num2str(height_num), '-Unbias.xlsx');
        folder_name = 'Up_and_Low';
        file_path = fullfile(pwd, folder_name, filename);
        Data = readmatrix(file_path,'Sheet', 'Avg_vector');
        B_j = Data((3*(sensor-1)+1):3*sensor)*1e-4;
        B_meas = [B_meas;B_j'];
...
M_0 = 0.04;
O_0 = eye(3);
[xyz_s_opt,O_opt,M_opt] = Sensor_backward_estimation_soft(B_meas,pm_all_all,xyz_i_s_0,O_0,meas_dir_i_s_0,M_0);

B_measure_theoretical = Sensor_forward(pm_all_all,xyz_s_opt,meas_dir_i_s_0,M_opt);
B_meas_mod = mod_B_meas_sensor(O_opt,B_meas);
B_err = B_meas_mod - B_measure_theoretical;

%%
[J] = Analytic_Jacobian_Sensor_cir_array_soft(pm_all_all,xyz_i_s_0,meas_dir_i_s_0,M_exp,B_meas);
...
M_all = [];
O_all = [];
sensor_all = [1:12,14:16];

for jj = 1:n_sens  % iterated among all sensors
sensor = sensor_all(jj);
% extract its position and orientation  as initial guess
xyz_i_s_0 = xyz_all(:,jj);
meas_dir_i_s_0 = meas_dir_all_s(:,(3*(jj-1)+1):3*jj);
...
for i = 1:length(col_labels)
    ...
    for j = num_rows
        ...
        phi_pm = -pi/2- deg2rad(tilt_angle);  % orientation
        % extract pm positions
        pm_all = [Single_PM_Location(magnet_type, col_letter, j , height);theta_pm;phi_pm];
        pm_all_all = [pm_all_all pm_all];
...
M_0 = M_exp;
O_0 = eye(3);
[xyz_s_opt,O_opt,M_opt] = Sensor_backward_estimation_soft(y_meas,pm_all_all,xyz_i_s_0,O_0,meas_dir_i_s_0,M_0);
...
for i = 1:length(col_labels)
    ...
    for j = num_rows
        ...
        pm_pos_true = Single_PM_Location(magnet_type, col_letter, j, height);
...
%         pm_true = [pm_pos_true;pm_ori_true];
        ...
        B_meas = B_meas([1:36,40:end]);
        % use true position as a initial guess
        pm_hat_0 = pm_true;   % (x_hat0,y_hat0,z_hat0,theta_hat0,phi_hat0,M0)
        
        % modify the measurements for each sensor
        B_mod = mode_B_meas_loc(O_all,B_meas);
        
        
        pm_hat_opt = PM_backward_estimation(B_mod,pm_hat_0,xyz_s_calibrated_all,meas_dir_all_s);
...
M_all = M_exp*6/10:5e-4:M_exp*10/10;
y_err_all = [];
y_err_norm_all = [];
...
[xyz_s_opt,O_opt,M_opt] = Sensor_backward_estimation_soft(y_meas,pm_all_all,xyz_i_s_0,O_0,meas_dir_i_s_0,M_k);
...
y_err = y_err + norm(y_meas - y_measure_theoretical,2);
