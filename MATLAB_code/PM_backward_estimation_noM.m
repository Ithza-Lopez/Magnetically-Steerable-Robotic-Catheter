function [pm] = PM_backward_estimation_noM(y_meas,pm_init,xyz_s,meas_dir_s,M)
...
J = Analytic_Jacobian_Mag_cir_array_noM(pm_init,xyz_s,meas_dir_s,M);
...
while ( ~stop && iteration <= MaxIter)
    ...
    B_meas = PM_forward_field_noM(pm,xyz_s,meas_dir_s,M);  % compute measured component
    ...
    B_meas_try = PM_forward_field_noM(pm_try,xyz_s,meas_dir_s,M);  % compute measurement based on given direction
    ...
    if rho > epsilon_4   % if better
        ...
        J = Analytic_Jacobian_Mag_cir_array_noM(pm,xyz_s,meas_dir_s,M);
    else      % if not better
        lambda = lambda*v;
        v = 2*v;
    end
